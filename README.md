...
- **Безопасные повторы** — экспоненциальный backoff с jitter и автоматический ключ идемпотентности `payload_id` для `sendText`, `sendSticker`, `sendSystemMessage` и `createPoll`, чтобы повторная попытка не отправила сообщение дважды. Для multipart-загрузок (`sendFile`, `sendImage`, `sendGallery`) API ключа идемпотентности не предоставляет — их повтор может создать дубль
